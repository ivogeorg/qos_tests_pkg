...
public:
  PublisherQoS(int &argc, char **argv) : Node("publisher_qos_obj") {
    ...
    // Note that PublisherOptions has an incompatible_qos_callback option!
  ...
  void
  incompatible_qos_info_callback(rclcpp::QOSOfferedIncompatibleQoSInfo &event) { // Note the parameter!
    RCLCPP_ERROR(this->get_logger(),
                 "A subscriber asking for an INCOMPATIBLE QoS triggered!");
    RCLCPP_ERROR(
        this->get_logger(),
