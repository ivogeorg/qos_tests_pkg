...
public:
  PublisherQoS(int &argc, char **argv) : Node("publisher_qos_obj") {
    ...
    group_pub_timer = this->create_callback_group(
    ...
    // Note: 1 callback group for the main code (events and non-timer callbacks)
    ...
    // Note: 2 callback groups for timer callbacks
    pub_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(publish_period),
        std::bind(&PublisherQoS::pub_callback, this), group_pub_timer);
...
private:
  ...
  rclcpp::CallbackGroup::SharedPtr group_pub_timer;
...
int main(int argc, char *argv[]) {
  ...
  // 2. 3 threads: 2 for the callbacks and 1 for the node main
